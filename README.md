...
| 10 | **重绘剩余两个抠图** | 温度计盘和指北条已重绘。剩下希卡感应器盘、声音盘两个仍是游戏原图直接抠的 —— 这两个在照片上纯装饰，也可以直接去掉而不必重画 |
...
需要说明的是，开发过程中会用一张游戏截图作为排版参照，从中裁出的元件放在本地的 `ui_source/` 目录（已 gitignore，不会进入版本控制）。目前 HUD 上仍有 2 个元件（希卡感应器盘、声音盘）直接使用这些裁图，成品发布前会替换或去掉。温度计盘和指北条已改为自绘。
