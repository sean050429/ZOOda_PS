...
上传一张照片，程序读出拍摄地点，在照片上叠一整套旷野之息风格的 HUD：左上是心心血条与装备槽，右下是希卡圆盘、拍摄时间、天气条和一张按当地地图生成的圆形小地图。
...
可调项：HUD 整体大小、心心总数与剩余数、小地图的地理范围/色阶/配色/箭头朝向/箭头大小、地名文字、时间文字、天气文字。
...
| # | 功能 | 说明 |
|---|---|---|
| 6 | **地名标题** | 地名已经能解析出来（见第 3 项），缺的是画到照片上：风格化字体、位置/大小/内容可调。需先选定免费字体（BOTW 原版字体有版权，不能直接用） |
| 7 | **天气状态栏** | 右下角目前是**文字占位**（如「晴 26°C」，可手改）。要接 Open-Meteo 按坐标 + 拍摄时间查历史天气，再换成图标。EXIF 解析已经算好了 UTC 时刻，直接可用 |
| 8 | **重绘剩余游戏素材** | HUD 上 11 个元件里，7 个已是矢量重绘，4 个（三个希卡圆盘、指北条）仍是游戏原图直接抠的，成品发布前必须换掉 |
| 9 | **导出成品** | 按原图分辨率重绘所有图层，导出高清 PNG。小地图的服务端渲染路径已经备好（`draw_player_marker()`），与前端共用同一组常量 |
| 10 | **其他游戏模版** | 文档里的拓展功能，架构上预留 |
